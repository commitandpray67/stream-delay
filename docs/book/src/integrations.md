...
| Remove delay now | `Ctrl+Alt+Shift+L` |
| Remove delay after it airs | `Ctrl+Alt+Shift+A` |
| Dump buffer | none; set one on the Advanced tab |
| End stream (after the buffer airs) | none; set one on the Advanced tab |
...
presets, **Remove the delay once the buffer has aired**, **Dump the buffer**, **End
stream once the buffer has aired**, **End stream now** and **Resume broadcasting**,
links to the dashboard and OBS setup, **Copy**
...
streamdelayd live               # remove the delay now
streamdelayd live --after-air   # remove it once what is buffered has aired
streamdelayd dump               # throw away what has not aired; keep streaming
streamdelayd end --after-air    # end the broadcast once the buffer has aired
...
| Remove delay | `POST http://127.0.0.1:7788/api/v1/live`, body `{"when": "now"}` |
| Remove delay after it airs | `POST http://127.0.0.1:7788/api/v1/live`, body `{"when": "after-air"}` |
| Dump buffer | `POST http://127.0.0.1:7788/api/v1/stream/dump` |
| End stream | `POST http://127.0.0.1:7788/api/v1/stream/end`, body `{"when": "after-air"}` |
| End stream now | `POST http://127.0.0.1:7788/api/v1/stream/end`, body `{"when": "now"}` |
